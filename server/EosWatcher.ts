...
import {EosRpcActionReader} from "./EosRpcActionReader";
import {EosMongoActionReader} from "./EosMongoActionReader";
...
class EosActionHandler extends AbstractActionHandler {
    ...
    protected rollbackTo(blockNumber: number): Promise<void> {
        ...
        if (this.rollbackToCallback) {
            return this.rollbackToCallback(blockNumber);
        } else {
        ...
        // let toRet:Promise<void> = new Promise<void>((resolve, reject) => {
        //     const indexState:IndexState = <IndexState> this.state.indexState;
        //     const toDelete = [...Array(indexState.blockNumber - (blockNumber)).keys()].map(n => n + blockNumber + 1)
        //     for (const n of toDelete) {
        //         delete this.stateHistory[n]
...
export class EosWatcher {
     ...
     * @param {EosWatcherCallbacks} callbacks
     * @param {(blockNumber: number, timestamp: string) => Promise<any>} processedBlockCallback
     * @param {(blockNumber: number) => Promise<any>} rollbackToCallback
     * @param {DBNodeos} dbNodeos
     * @param {boolean} onlyReversible
     * @param {number} maxHistoryLength
     * @param requestInstance
     */
    constructor (endPoint:string, startingBlock:number, callbacks:EosWatcherCallbacks,
                 ...
                 dbNodeos:DBNodeos = null,
                 onlyReversible?:boolean,
                 maxHistoryLength?:number,
                 requestInstance?:any) {

        this.dbNodeos = dbNodeos;
        let actionReader:AbstractActionReader = dbNodeos ? new EosMongoActionReader(dbNodeos, startingBlock, onlyReversible, maxHistoryLength, processedBlockCallback) : new EosRpcActionReader(endPoint, startingBlock, onlyReversible, maxHistoryLength, processedBlockCallback);
        let actionHandler:EosActionHandler = new EosActionHandler(callbacks.getUpdaters(), callbacks.getEffects(), rollbackToCallback);
        this.actionWatcher = new BaseActionWatcher(actionReader, actionHandler, 2000);
    }
